...
setwd("/Users/hannaaro/Desktop/machinelearning")

#Importazione dataset

c <- read.csv("dataset_banca.csv", sep = "," , dec = ".",  
...
#Il dataset non è stato bilanciato in precedenza e l'evento 0 di Exited non è raro, quindi non dobbiamo fare niente a riguardo
#Le fonti confermano che la distribuzione di Exited nel dataset è la stessa della popolazione
...
#Unica chi square alta è tra Exited e Complain = 0.99 : rimuoviamo Complain
s2 <- r[,-12]
...
table(r$Exited, r$Card.Type) / nrow(r) # va bene
...
#5) outlier: serve per logistico e reti ma serve il modello ecc... quindi se vediamo che i modelli non sono performanti potrebbe essere colpa degli outlier e torniamo indietro e lo facciamo (magari con preProcess di caret)
#6) std/norm variabili: lasso, reti neurali, pls, knn
#7) separation/zero-problem: logistico, nb => ma non ce ne sono quindi a posto 
?preProcess
#Soluzione: due dataset (di training): uno per tree, rf, gb; l'altro per gli altri 

#NB: CODIFICA DEL TARGET
...
#CARET LEGGE COME CLASSE 1 LA CLASSE C0 => TUNARE PER LA SENS = TUNARE PER C0 CIOÉ PER QUELLI CHE CHURNANO

#Caso peggiore: classificare come 1 gli 0 (cioè classificare come "non exit" gli "exit") perchè la banca non riconosce quali clienti perderà, quindi minimizziamo i falsi negativi
#Quindi tuniamo con la Sensitivity così da allenare il modello a classificare bene i true positive, cioè quelli che lasceranno la banca
...
head(var_imp_rf)
plot(var_imp_rf) #SI PRENDE UNA SOGLIA E SI USA RF COME MODEL SELECTOR: LE VAR CON VAR IMP OLTRE LA SOGLIA SI DANNO IN PASTO AD ALTRI ALGORITMI CHE RICHIEDONO MODEL SELECTION
...
gbm_grid <- expand.grid(n.trees = 200, interaction.depth = c(1:9), shrinkage = c(0.075, 0.1, 0.5, 0.7), n.minobsinnode = 20)
...
#Plottiamo i box-plot dell'importanza (MDI) delle variabili (sono box-plot a causa delle varie iterazioni, quindi l'MDI ha una distribuzione)
...
bwplot(results)
dotplot(results)
###nnet, rf, bagging, gbm sono i migliori sulla Sens; li useremo quindi nell'ensemble
...
#Creiamo una lista, convertita poi in formato caretList per darla in pasto a caretEnsamble 
...
# scegli i modelli con le migliori prestazioni sui primi 2,3,4 decili 
...
#guardando il quarto decile il moglior modello: nnet e rf, scegliamo rf perchè a parità di performance classificative è maggiormente interpretabile di nnet
#nel 4° decile (quindi con il 40% dei dati) la random forest classifica correttamente come c0 (churner) l'80% dei casi
...
#usiamo decision rule per il modello migliore #####
...
#Un Partial dependence profile dice di più rispetto alla variable importance della RF, ovvera la direzione e l'andamento della connessione tra covaraite e target
...
sv6  <- single_variable(explainer, variable = "Age",  type = "pdb", prob = T)
plot(sv6)

sv7  <- single_variable(explainer, variable = "NumOfProducts",  type = "pdb", prob = T)
plot(sv7)
...
#Ora valutiamo l'effettO delle variabili sulla prediction della RF su una nuova singola osservazione presa dal dataset di score
